...
function applyStatus(data) {
  ...
  document.getElementById("onlineCount").textContent =
    data.player_count + " online";
  statusStartedAt =
    data.uptime_seconds != null
      ? Date.now() - data.uptime_seconds * 1000
      : null;
...
setInterval(() => {
  if (statusStartedAt != null)
    document.getElementById("status").textContent = formatUptime(
      (Date.now() - statusStartedAt) / 1000,
    );
}, 30000);
...
function applyActivity(a) {
  const byMode = Object.fromEntries(
    (a.by_mode || []).map((m) => [m.mode_id, m.players]),
  );
  setFigure("figIdle", a.in_menu);
  setFigure("figSpectating", a.spectating || 0);
  for (const [id, modeId] of Object.entries(FIG_MODES))
    setFigure(id, byMode[modeId] || 0);
}
...
function renderQueues() {
  ...
  if (queuesStale) {
    box.innerHTML =
      '<div class="queue-col"><p class="queue-empty error-row">' +
      "Cannot read the matchmaker right now.</p></div>";
    return;
  }
  if (!queues) {
    box.innerHTML =
      '<div class="queue-col"><p class="queue-empty">Loading…</p></div>';
    return;
  }
  if (!queues.length) {
    box.innerHTML =
      '<div class="queue-col"><p class="queue-empty">' +
      "No matchmaking modes in the catalogue.</p></div>";
    return;
  }

  const now = Date.now();
  box.innerHTML = queues
    .map((q) => {
      ...
      return (
        ...
        "<span>" +
        esc(q.mode_name || q.mode_id) +
        "</span>" +
        queueState(q, now) +
        '<span class="queue-col-count">' +
        q.player_count +
        "</span>" +
        "</div>" +
        (players.length
...
function queueState(q, now) {
  ...
  const left =
    q.confirm_deadline != null
      ? " " +
        Math.max(0, Math.round((Number(q.confirm_deadline) - now) / 1000)) +
        "s"
      : "";
  const label =
    {
      ...
      pop_failed: "pop failed",
      unavailable: "unavailable",
    }[q.state] || q.state;
  return (
    '<span class="queue-state queue-state-' +
    esc(q.state) +
    '">' +
    esc(label) +
    left +
...
registerView("status", {
  enter: () => {
    statusRenderTimer = setInterval(() => {
      if (queues) renderQueues();
    }, 1000);
  },
  leave: () => {
