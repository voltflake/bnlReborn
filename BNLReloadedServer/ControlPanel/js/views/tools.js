...
async function toggleMatchmaking() {
  ...
  try {
    ...
    showToast(
      "success",
      data.enabled ? "Matchmaking enabled." : "Matchmaking disabled.",
    );
...
async function resetServer() {
  if (
    !confirm(
      "This shuts down the server process, disconnecting all players. The service is expected to relaunch it automatically. Continue?",
    )
  )
    return;
  const btn = event.target;
...
async function lookupCard() {
  ...
  if (!query) {
    showToast("error", "Enter a card key or key hash.");
    return;
  }
  ...
  try {
    ...
    lookedUpCardName = String(data._id || data.id || query).replace(
      /[^a-z0-9._-]+/gi,
      "_",
    );
